...
function get_line_area (data, size) {
  ...
  for (var i = 1; i < data.length; i++) {
    area += Math.sqrt(
      ((data[i]['x'] - data[i - 1]['x']) / size[0]) ** 2 +
        ((data[i]['y'] - data[i - 1]['y']) / size[1]) ** 2
    )
  }
  return area
}
...
function check_and_get_property (parsed_data, attr) {
  ...
  return attr in parsed_data['properties']
    ? parseFloat(parsed_data['properties'][attr]) > 0
      ? parseFloat(parsed_data['properties'][attr])
      : 0
    : 0
}
...
function rgbstring2obj (rgb, prefix) {
  if (!rgb.includes('rgb') && prefix === 'fill') {
    return no_fill_color
  }
  if (!rgb.includes('rgb') && prefix === 'stroke') {
    return no_stroke_color
  }
...
function get_text_feature (parsed_data, translate, size) {
  var len = parsed_data['properties']['data-unformatted']
    ? parsed_data['properties']['data-unformatted'].toString().length
    : 0
...
function get_path_feature (parsed_data, translate, size) {
  ...
  if (parsed_data['properties']['class']) {
    if (
      !parsed_data['properties']['d'].includes('Z') &&
      !['js-fill', 'openline', 'point', 'mean'].includes(
        parsed_data['properties']['class']
      ) &&
      !parsed_data['properties']['class'].includes('crisp') &&
      !parsed_data['properties']['class'].includes('error')
    ) {
      var pathData = new SVGPathData(parsed_data['properties']['d'])

      var data = pathData.commands
        .filter(d => 'x' in d && 'y' in d)
        .map(d => {
          return {
            x: d.x / size[0],
            y: d.y / size[1]
      ...
      if (feature_trend.length > 5) {
        let list_keep = [
          Math.round(feature_trend.length * 0.0),
          Math.round((feature_trend.length - 1) * 0.25),
          Math.round((feature_trend.length - 1) * 0.5),
          Math.round((feature_trend.length - 1) * 0.75),
          Math.floor((feature_trend.length - 1) * 1)
        ]

        var feature_trend_f = feature_trend
          .filter((d, i) => {
            return list_keep.includes(i)
          })
          .map(d => d[1])
        if (feature_trend_f.length !== 5)
          console.log('if', list_keep, feature_trend_f.length)
      } else {
        var feature_trend_f = feature_trend
          .map(d => d[1])
          .concat(Array(5 - feature_trend.length).fill(0))
        if (feature_trend_f.length !== 5)
          console.log(
            'else',
            feature_trend_f.length,
            Array(5 - data_length).fill(0)
          )
      }
      ...
      return {
        ...
        length: 0,
        num_vertex:
          parsed_data['properties']['d'].match(/[a-zA-Z]/g).length >
          max_vertex_num
            ? 1
            : parsed_data['properties']['d'].match(/[a-zA-Z]/g).length /
              max_vertex_num,
...
function get_non_empty_g (
  ...
  if (
    disposed_class.includes(parsed_data['properties']['class']) ||
    parsed_data['tagName'] == 'defs'
  ) {
    return 0
  }
  ...
  if (
    (parsed_data['tagName'] == 'path' &&
      parsed_data['properties']['d'] == 'M0,0') ||
    (parsed_data['tagName'] == 'path' && !('d' in parsed_data['properties'])) ||
    (parsed_data['tagName'] == 'path' &&
      parsed_data['properties']['d'].length < 1)
  ) {
    return 0
  }
  ...
  if (
    parsed_data['children'].length === 0 &&
    parsed_data['tagName'] === 'svg'
  ) {
    return 0
  } else {
    // if translate, plus
    if ('transform' in parsed_data['properties']) {
      if (parsed_data['properties']['transform'].includes('translate')) {
        var translate_string = parsed_data['properties']['transform']
          .split(')')[0]
          .split('(')[1]

        translate = [
          parseFloat(translate_string.split(',')[0]) + translate[0],
        ...
        if (
          !(parsed_data['tagName'] === 'text') &&
          !list_unable_resolve.includes(filename) &&
          ...
          if ('class' in parsed_data['properties']) {
            if (!parsed_data['properties']['class'].includes('annotation'))
              list_unable_resolve.push(filename)
    ...
    if (
      'width' in parsed_data['properties'] &&
      parsed_data['tagName'] === 'svg'
    ) {
      size = [
        parseFloat(parsed_data['properties']['width']),
        parseFloat(parsed_data['properties']['height'])
      ]
    }

    for (var i in parsed_data['children']) {
      var return_value = get_non_empty_g(
        parsed_data['children'][i],
        filename,
        translate,
        size
      )
  ...
  if (list_non_empty.length > 0 && parsed_data['tagName'] === 'g') {
    var type =
      parsed_data['tagName'] == undefined ? 'un' : parsed_data['tagName']
    var class_name =
      parsed_data['properties']['class'] == undefined
        ? 'un'
        : parsed_data['properties']['class']
    ...
    if (
      parsed_data['properties']['class'] == 'points' ||
      parsed_data['properties']['class'] == 'lines'
    ) {
      if (list_non_empty[0]['children'].length == 0) {
        ...
        for (var i in list_non_empty) {
          if (i == 0) {
            ...
            list_non_empty[i]['features']['pre_node'] =
              list_non_empty[i - 1]['id']
            list_non_empty[i]['features']['delta_x'] =
              list_non_empty[i]['features']['cx'] -
              list_non_empty[i - 1]['features']['cx']
            list_non_empty[i]['features']['delta_y'] =
              list_non_empty[i]['features']['cy'] -
              list_non_empty[i - 1]['features']['cy']
          }
        }
      } else {
        list_non_empty.sort((a, b) => {
          if (
            a['children'][0]['features']['cx'] !==
            b['children'][0]['features']['cx']
          ) {
            return (
              a['children'][0]['features']['cx'] -
              b['children'][0]['features']['cx']
            ...
            return (
              a['children'][0]['features']['cy'] -
              b['children'][0]['features']['cy']
            )
          }
        })
        ...
        for (var i in list_non_empty) {
          ...
          if (i == 0) {
            ...
            list_non_empty[i]['children'][0]['features']['pre_node'] =
              list_non_empty[i - 1]['children'][0]['id']
            list_non_empty[i]['children'][0]['features']['delta_x'] =
              list_non_empty[i]['children'][0]['features']['cx'] -
              list_non_empty[i - 1]['children'][0]['features']['cx']
            list_non_empty[i]['children'][0]['features']['delta_y'] =
              list_non_empty[i]['children'][0]['features']['cy'] -
              list_non_empty[i - 1]['children'][0]['features']['cy']
          }
        }
      }
    }
    ...
    var stroke_color =
      'stroke' in css_json['attributes'] &&
      css_json['attributes']['stroke'] !== 'none'
        ? rgbstring2obj(css_json['attributes']['stroke'], 'stroke')
        : no_stroke_color

    var fill_color =
      'fill' in css_json['attributes'] &&
      css_json['attributes']['fill'] !== 'none'
        ? rgbstring2obj(css_json['attributes']['fill'], 'fill')
        : no_fill_color

    if (parsed_data['tagName'] in get_feature) {
      var element_feature = get_feature[parsed_data['tagName']](
        parsed_data,
        translate,
        size
      )
    ...
    if (Array.isArray(element_feature)) {
      ...
      for (var i in element_feature) {
        if (i == 0) {
          ...
          element_feature[i]['delta_x'] =
            element_feature[i]['cx'] - element_feature[i - 1]['cx']
          element_feature[i]['delta_y'] =
            element_feature[i]['cy'] - element_feature[i - 1]['cy']
        }
        parsed_data['children'].push({
          ...
          features: {
            ...
            // (0, 1)
            fill_opacity:
              'fill-opacity' in css_json['attributes']
                ? parseFloat(css_json['attributes']['fill-opacity'])
                : 0,
            // (0, 1)
            stroke_opacity:
              'stroke-opacity' in css_json['attributes']
                ? parseFloat(css_json['attributes']['stroke-opacity'])
                : 0,
            // normalized
            stroke_width:
              'stroke-width' in css_json['attributes']
                ? size[1] > size[0]
                  ? parseFloat(
                      css_json['attributes']['stroke-width'].split('px')[0]
                    ) / size[1]
                  : parseFloat(
                      css_json['attributes']['stroke-width'].split('px')[0]
                    ) / size[0]
  ...
  files.forEach(function (file, index) {
    ...
    if (file.includes('.svg')) {
      fs.readFile(fromPath, 'utf8', (err, data) => {
        ...
        try {
          var data_processed_1 =
            data.split('<defs')[0] +
            '<g class="cartesianlayer">' +
            data
              .split('</svg>')[0]
              .split('<g class="cartesianlayer">')[1]
              .split('<g class="polarlayer">')[0] +
            '</svg>'
          var data_processed_2 =
            data.split('</svg>')[1].split('</div>')[1] + '</svg>'
        } catch (error) {
          console.log(file)
          console.error(error)
        }
        ...
        fs.writeFile(
          `debug_${type}.json`,
          JSON.stringify({
            transform_unresovable: list_unable_resolve,
            null_feature: list_with_null,
            exceed_max_char: num_exceed_max_char
          }),
          function (err, data) {
            if (err) {
        ...
        try {
          fs.writeFile(
            `${output_path}/${file.slice(0, -4)}.json`,
            JSON.stringify(write_data),
            function (err, data) {
              if (err) {
                return console.log(err)
              }
